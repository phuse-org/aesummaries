...
title_ftnote<-function(domain,summary_by,filters,report){
  ...
  if (summary_by == "Events") {
    ...
    if (is.null(filters)){
      ...
      if (length(filter_name)<=1){
        ftn <- paste("* n is the number of ",filter_name,"adverse events.")  
      } else if (length(filter_name)>1){
        ftn <- paste0("* n is the number of ",paste0(filter_name[-length(filter_name)],collapse=", "), " and ", filter_name[length(filter_name)], " adverse events.")
      }
    }
  }
  if (summary_by == "Patients") {
    ...
    if (is.null(filters)){
      ...
      filter_name = tolower(filters)
      if (length(filter_name)<=1){
        ftn <- paste("* n is the number of patients with ",filter_name,"adverse events.")  
        
      } else if (length(filter_name)>1){
        ftn <- paste0("* n is the number of patients with ",paste0(filter_name[-length(filter_name)],collapse=", "), " and ", filter_name[length(filter_name)], " adverse events.")
      }
    }
  }
    f <- paste0("Note: \n",ftn, " \n",f1," \n",f2)
