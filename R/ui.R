...
ui <- fluidPage(tabsetPanel(
  ...
  column(width=4,
         wellPanel(
           fluidRow(column(width=5,style = "font-size: 12px;",selectInput("domain","Domain",choices = c("AE", "LB"))),
                    column(width=5,style = "font-size: 12px;",
                           fluidRow(conditionalPanel(condition = 'input.domain=="AE"',
                                                     selectInput('report','Report',c("Volcano","Forest")))),
                           conditionalPanel(condition = 'input.domain=="LB"',
                                            selectInput('report1','Report',c("Edish"))))
           )),
         wellPanel(div(style = 'height:120px;',fluidRow(column(width=5,style = "font-size: 12px;",selectInput("source","Data Source",choices = c("Local","Server"))) ,
                            column(width=6,style = "font-size: 12px;",
         ...
         conditionalPanel(condition='input.domain=="LB"',
                          wellPanel(fluidRow(column(width=12,style = "font-size: 12px;",
                                             ...
                                             h6('X Axis Options'),
                                             column(width=6,style = "font-size: 12px;",
                                                    textInput("xbreaks","Breaks",value="0.1,1,2,10")),
                                             column(width=6,style = "font-size: 12px;",
                                                    textInput("xlimits","Limits",value="0.1,10")),br(),
                                             column(width=6,style = "font-size: 12px;",
                                                    textInput("xticklbl","Tick Label",value="0.1,1,2x ULN,10")),
                                             column(width=6,style = "font-size: 12px;",
                                                    textInput("xaxislbl","Axis Label",value="Peak AST or ALT (x ULN)")),br(),
                                             h6('Y Axis Options'),
                                             column(width=6,style = "font-size: 12px;",
                                                    textInput("ybreaks","Breaks",value="0.1,1,3,10")),
                                             column(width=6,style = "font-size: 12px;",
                                                    textInput("ylimits","Limits",value="0.1,10")),br(),
                                             column(width=6,style = "font-size: 12px;",
                                                    textInput("yticklbl","Tick Label",value="0.1,1,3x ULN,10")),
                                             column(width=6,style = "font-size: 12px;",
                                                    textInput("yaxislbl","Axis Label",value="Peak Bilirubin (x ULN)")),br(),
                                             h6('X Axis Ref Line'),
                                             column(width=4,style = "font-size: 12px;",
                                                    textInput("rlxintercept","Intercept",value="2")),
                                             column(width=4,style = "font-size: 12px;",
                                                    textInput("rlxcolor","Color",value="gray30")),
                                             column(width=4,style = "font-size: 12px;",
                                                    textInput("rlxlinetyp","Line Type",value="dashed")),br(),
                                             h6('Y Axis Ref Line'),
                                             column(width=4,style = "font-size: 12px;",
                                                    textInput("rlyintercept","Intercept",value="3")),
                                             column(width=4,style = "font-size: 12px;",
                                                    textInput("rlycolor","Color",value="gray30")),
                                             column(width=4,style = "font-size: 12px;",
          ...
          conditionalPanel(condition='input.domain=="AE"',
                           wellPanel(fluidRow(column(width=12,style = "font-size: 12px;",
                                    ...
                                    fluidRow(style = "font-size: 12px;",selectInput("sort_opt", "Sorting Option", choices = c("Ascending Count", "Descending Count", "Alphabetical"), width="75%"),
                                             ),hr(),
                                    fluidRow(style = "font-size: 12px;",selectInput("statistics", "Measure of Association", choices = c("Risk Ratio", "Risk Difference"), width="75%"),
                                             ),hr(),
                                    fluidRow(column(width=6,style = "font-size: 12px;",uiOutput("treatment1_UI"),  uiOutput("treatment1_label_UI")),
                                             column(width=6,style = "font-size: 12px;",uiOutput("treatment2_UI"),  uiOutput("treatment2_label_UI"))
                                             ),br(),hr(),
                                    fluidRow(column(width=6,style = "font-size: 12px;",numericInput("X_ref", "X-axis Reference Lines", value = 0,width="75%") ),
                                             column(width=6,style = "font-size: 12px;",selectInput("pvalue_label", "P-value Transformation",choices = c("None", "-log10"), selected="None",width="75%"))
                                             ),br(),
                                    fluidRow(column(align="center",width=6,style = "font-size: 12px;",numericInput("alpha", "Alpha Value(CI)", value = 0.05, min = 0.01, max = 0.1))
                                             )
                          )))  ,
  column(width=8,
         fluidRow(column(width=12,div(htmlOutput("title_UI"),style = "font-size: 12px; white-space: pre"),
                         uiOutput("plot_UI"))),
         fluidRow(column(width=12,div(htmlOutput("footnote_UI"),style = "font-size: 12px; white-space: pre"))),
         fluidRow(column(width=12,DT::dataTableOutput("plot_drill")))
  )
)
))
