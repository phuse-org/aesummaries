...
server <- function(input, output, session) {
  ...
  data <- reactiveValues(domain=NULL,
                         data_in = NULL,
                         statistics = NULL,
                         statistics_soc = NULL)
  ...
  analysis_data <- reactive({
    # SDTM
    withProgress(df<-data_processing(datain=input$analysis_data,
                        domain=input$domain,
  ...
  output$fileUploaded <- reactive({ return(!is.null(analysis_data())) })
  ...
  ARMCD <- reactive({
    req(data$data_in)  
    unique(data$data_in$TRTA[data$data_in$TRTA!=""]) 
  })
  ...
  output$treatment1_label_UI <- renderUI({
    req(data$data_in) 
    req(input$summary_by != "Events")
    textInput("treatment1_label", "Label for Group A", value="Unexposed",width="75%")
  })
  
  output$treatment2_label_UI <- renderUI({
    req(data$data_in) 
    req(input$summary_by != "Events")
    textInput("treatment2_label", "Label for Group B",value="Exposed",width="75%")
  })
  ...
  observeEvent(input$obtain, {
    ...
    if (input$review_by != "SOC"){
      if (!is.null(analysis_data())) { data$statistics <- analysis_data()   
      } else if(!is.null(analysis_data())) {data$statistics <- analysis_data() }
    }
    if (input$summary_by == "Events") {
    if (input$review_by != "SOC"){
      withProgress(data$statistics <- GetStatistics_all(data = analysis_data(),
    ...
    withProgress(message = 'Generating Volcano Plot', value = 0, {
      ...
      withProgress(message = 'Generating Forest Plot', value = 0, {
        out <- try(
          Forest_Plot(dat=isolate(data$statistics),
                      groupvar="ARM",
                      review_by=input$review_by,
                      summary_by=input$summary_by,
                      xlims=c(0,3),
    ...
    if(input$domain=="LB"){
      withProgress(message = 'Generating Edish Plot', value = 0, {
        out <- try(edish(datain=analysis_data(),
                         subset=input$subset,
                         xaxisopt=c(input$xbreaks,input$xlimits,input$xticklbl,input$xaxislbl),
                         yaxisopt=c(input$ybreaks,input$ylimits,input$yticklbl,input$yaxislbl),
    ...
    ft_out<-title_ftnote(domain=input$domain,
                         summary_by=input$summary_by,
        ...
        }else {plots$drill<-out$plot} 
        plots$title <- ft_out[1]
        plots$ft <- ft_out[2]
        plots$plot_data <- out$data
        plots$ptly<-out$ptly
        output$plot_output <- renderPlotly({out$ptly})
  ...
  output$obtain_UI <- renderUI({
    req(data$data_in)
    div(style = 'height:6px;font-size: 5px;',actionBttn("obtain", "Create",color = "primary",style='simple'))
  })
  ...
  output$plot_drill <- DT::renderDataTable({
    ...
    if(input$domain=="AE"){
      display <- analysis_data() %>% 
        select(any_of(c("USUBJID","TRTA","AEBODSYS","AEDECOD","AESER","AEOUT","AESTDT","ASTTM","AEENDT",
                        "TRTSTDT","TRTEDT","TRTEMFL")))
      if (input$review_by=="PT"){
        plot_table = select(test,'AEBODSYS','AEDECOD','ARM')%>% 
          inner_join(display)
      }
      if(input$review_by=="SOC"){
        plot_table = select(test,'AEBODSYS','ARM')%>% 
          inner_join(display)
      }
    }
    if(input$domain=="LB") {
      display <- analysis_data() %>% 
        select(any_of(c("USUBJID","TRTA","PARAM","PARAMCD","AVAL","ADT",
                        "ANRIND","ANRHI","ANRLO","AVISIT","APBFL")),starts_with("ANL0"))
      plot_table = select(test,'USUBJID','TRTA')%>% 
        inner_join(display)
    }
    plot_table<-plot_table %>% distinct()
    datatable(plot_table, extensions = 'Buttons',
  ...
  output$dplot <- downloadHandler(
    filename = function() {
      paste0(ifelse(input$domain=="AE",input$report,input$report1), Sys.Date(), ".",input$fmt)
    },
    content = function(file){
      title <- ggdraw() + draw_label(plots$title,size=12, x = 0.05, hjust = 0,y=0.5)
      foot <- ggdraw() + draw_label(plots$ft,size=9,x=0.05,hjust=0,y=0.5)
      pdfout <- plot_grid(title,plots$plot_output,foot,rel_heights = c(0.05,0.9,0.1),ncol=1)
      #pdfout <- add_sub(pdfout1,plots$ft,y=0.5,x=0.05,hjust=0,size=10)
      if (input$fmt=="pdf"){
        #Save to pdf
        if (input$report=="Forest"){
          ploth <- max(15,((plots$n)*0.12)+2)
          ggsave(file,pdfout,height=ploth,width=15,device = "pdf",dpi=300,units = "in",limitsize = F)
        }
        if (input$report %in% c("Volcano","Edish")){
          ggsave(file,pdfout,device = "pdf",height = 15,width = 15,dpi=300,units="in")
        }
      }
      if (input$fmt %in% c("html","pptx")){
        ...
        if (input$report=="Forest"){
          ...
          if (input$review_by=="SOC"){w=15}else{w=12}
          ggsave(tf1,pdfout,height=ploth,width=w,dpi=100,units = "in")
        }
        if (input$report=="Edish"){
          ggsave(tf1,pdfout,height = 10,width = 12,dpi=100,units="in")
        }
        if (input$report=="Volcano"){
          ggsave(tf1,pdfout,height = 10,width = 15,dpi=100,units="in")
        }
      if (input$fmt=="html"){
        ...
        read_pptx() %>% add_slide() %>% 
          ph_with(external_img(tf1),ph_location_fullsize(left=0,top=0)) %>% 
          print(target=file)
      }
      }
    })
